...
- `total_play_count`: 対象`game_slug`のランキング登録行（`ranking_status`が`normal`、NULLは通常扱い）にある`game_scores.play_count`の合計です。
- `player_count`: 同じランキング登録行の数です。初回・ベストランキングと同じ通常状態の行を基準にします。

開始イベントだけがあり、まだスコアをランキングへ登録していない名前は、`total_play_count`にも`player_count`にも含めません。これにより、詳細ページの「合計プレイ回数」と「参加人数」をランキング登録者に一致させます。

20260915151357のマイグレーションで、イベント・セッション数ではなくランキング行を両方の集計元に統一しました。
