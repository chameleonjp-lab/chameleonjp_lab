...
`get_game_play_stats`は、詳細ページへ次の2つを返します。

- `total_play_count`: 既存の保存方式に従った合計プレイ回数。開始セッションとプレイイベントがあるゲームでは、それらを合算します。イベントがないゲームでは、`game_scores.play_count`を合計します。
- `player_count`: `public.game_scores`のうち、対象`game_slug`で`ranking_status`が`normal`（NULLは通常扱い）の行数。これは初回・ベストランキングと同じ、ランキング登録者の数です。

開始イベントだけがあり、まだスコアをランキングへ登録していない名前は、`total_play_count`には含まれても`player_count`には含めません。これにより、詳細ページの「参加人数」とランキング登録者数を一致させます。

この変更では、合計プレイ回数の保存元は変えず、参加人数の基準だけをランキングの通常行へそろえています。
