...
`合計プレイ回数`と`参加人数`は、対象ゲームのランキングへ登録された通常状態の行を基準にします。合計プレイ回数は `game_scores.play_count` の合計、参加人数はその行数です。開始イベントだけがあり、まだスコアをランキングへ登録していない名前は、どちらにも含めません。初回・ベストランキングと同じ `ranking_status = 'normal'`（NULLは通常扱い）の行を基準にします。
