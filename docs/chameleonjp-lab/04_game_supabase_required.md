...
名前と開始操作を受け付けたら、一つの`play_id`を作ります。

```js
function createId() {
  const randomUUID = globalThis.crypto?.randomUUID;
  if (typeof randomUUID !== "function") {
    throw new Error("送信IDを作れません。");
  }
...
4. 受付を確認してゲームを始める。
5. 一時的失敗の再送では同じ`play_id`を使う。
6. 開始ボタンの連打で新しい`play_id`を作らない。

ページを開いた時、名前を入力した時、チュートリアルを見た時には数えません。再戦で新しいゲームを始めた時は、新しい`play_id`を作ります。
